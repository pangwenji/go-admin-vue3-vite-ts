            ...
            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.isEdit" true-label="1" false-label="0" />
                    ...
                    <el-table-column
                        label="查询"
                        width="70"
                        :render-header="renderHeadeSearch"
                        :cell-style="{ 'text-align': 'center' }"
                    >
                        <template slot-scope="scope">
                            <el-checkbox
...
export default defineComponent({
    ...
    setup() {
        ...
        let route = useRoute();
        let renderHeadeUpdate = (h, { column, $index }) => {
        ...
        let renderHeadeList = (h, { column, $index }) => {
            // h 是一个渲染函数       column 是一个对象表示当前列      $index 第几列
            return h("div", [
                h("span", column.label + "  ", { align: "center", marginTop: "0px" }),
                h(
        ...
        let renderHeadeSearch = (h, { column, $index }) => {
            return h("div", [
                h("span", column.label + "  ", { align: "center", marginTop: "0px" }),
                h(
                    "el-popover",
        ...
        let handleChangeConfig = (row, index) => {
        ...
        let close = () => {
            ...
            router.push({ path: "/dev-tools/gen", query: { t: Date.now() } });
        };
        ...
        onMounted(() => {
            ...
            const { tableId } = route.query;
            if (tableId) {
                // 获取表详细信息
                getGenTable(tableId).then((res) => {
                    state.columns = res.data.list;
                    state.info = res.data.info;
                    state.info.isDataScope = state.info.isDataScope.toString();
                    state.info.isActions = state.info.isActions.toString();
                    state.info.isAuth = state.info.isAuth.toString();
                    state.columns.forEach((item) => {
                        state.tableTree.filter(function (e) {

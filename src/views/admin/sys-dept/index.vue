...
export default defineComponent({
    ...
    setup() {
        ...
        let statusFormat = (row) => {
            return selectDictLabel(state.statusOptions, parseInt(row.status));
        };
        // 取消按钮
        let cancel = () => {
            state.open = false;
        ...
        let handleUpdate = (row) => {
            ...
            getDept(row.deptId).then((response) => {
                ...
                state.form.sort = String(state.form.sort);
                state.open = true;
                state.title = "修改部门";
                state.isEdit = true;
            });
            ...
            ctx.$refs["form"].validate((valid) => {
                if (valid) {
                    ...
                    if (state.form.deptId !== undefined) {
                        updateDept(state.form, state.form.deptId).then((response) => {
        ...
        let handleDelete = (row) => {
            ...
            ctx.$confirm('是否确认删除名称为"' + row.deptName + '"的数据项?', "警告", {

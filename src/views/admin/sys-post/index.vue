    ...
    setup() {
        ...
        let getList = () => {
            ...
            listPost(state.queryParams).then((response) => {
                state.postList = response.data.list;
            ...
            ctx.$refs["form"].validate((valid) => {
                if (valid) {
                    ...
                    if (state.form.postId !== undefined) {
                        updatePost(state.form, state.form.postId).then((response) => {
        ...
        let handleDelete = (row) => {
            ...
            const Ids = (row.postId && [row.postId]) || state.ids;
            ctx.$confirm('是否确认删除岗位编号为"' + Ids + '"的数据项?', "警告", {

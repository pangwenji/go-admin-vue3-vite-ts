...
export default defineComponent({
    ...
    setup() {
        ...
        let getList = () => {
            state.loading = true;
            listUser(addDateRange(state.queryParams, state.dateRange)).then((response) => {
                state.userList = response.data.list;
        ...
        let handleStatusChange = (row) => {
            ...
            ctx.$confirm('确认要"' + text + '""' + row.username + '"用户吗?', "警告", {
        ...
        let handleUpdate = (row) => {
            ...
            listPost({ pageSize: 1000 }).then((response) => {
                state.postOptions = response.data.list;
            });
            listRole({ pageSize: 1000 }).then((response) => {
                state.roleOptions = response.data.list;
            });
        };
        /** 重置密码按钮操作 */
        let handleResetPwd = (row) => {
            ctx.$prompt('请输入"' + row.username + '"的新密码', "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
            })
                .then(({ value }) => {
            ...
            ctx.$refs["form"].validate((valid) => {
                if (valid) {
                    if (state.form.userId !== undefined) {
                        ...
                        addUser(state.form).then((response) => {
                            if (response.code === 200) {
                                msgSuccess(response.msg);
                                state.open = false;
                                getList();
        ...
        let handleDelete = (row) => {
            ...
            ctx.$confirm('是否确认删除用户编号为"' + Ids + '"的数据项?', "警告", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning",
            })
        ...
        // 文件上传中处理
        let handleFileUploadProgress = (event, file, fileList) => {
            state.upload.isUploading = true;
        };
        // 文件上传成功处理
        let handleFileSuccess = (response, file, fileList) => {
            state.upload.open = false;
            state.upload.isUploading = false;
            ctx.$refs.upload.clearFiles();
            ctx.$alert(response.msg, "导入结果", { dangerouslyUseHTMLString: true });
            getList();
        };
        // 提交上传文件
        let submitFileForm = () => {
        ...
        onMounted(() => {
            ...
            getConfigKey("sys_user_initPassword").then((response) => {
                state.initPassword = response.data.configValue;
            });
        });

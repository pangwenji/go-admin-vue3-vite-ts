...
export default defineComponent({
    ...
    setup() {
        ...
        let sys_app_logoBeforeUpload = (file) => {
        ...
        let uploadSuccess = (response, file, fileList) => {
            state.form.sys_app_logo = process.env.VUE_APP_BASE_API + response.data.full_path;
        };
        let setUrl = (url) => {
            const data = {
                sys_app_logo: "",
            ...
            // 回填数据
            fillFormData(this.formConf, data);
            // 更新表单
            state.key2 = +new Date();
        };
        // 参数系统内置字典翻译
